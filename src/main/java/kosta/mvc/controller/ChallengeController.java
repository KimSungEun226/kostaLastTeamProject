...
public class ChallengeController {
	...
	public String insert(Board board, int challengeCategory) {
		...
		//진행중인 challenge조회 
		Challenge ischallenge = challengeService.selectChallenge(challengeCategory, board.getMember().getMemberNo());
		if(ischallenge!=null) {
			//진행중인 챌린지가 있다. 
			
			board.setChallenge(ischallenge);
			//boardChallengeUpdate로 Service에서 해야하나??
			
			boardService.insert(board);
		}
		else {
			//진행중인 챌린지가 없으니 challenge생성하고 board insert
